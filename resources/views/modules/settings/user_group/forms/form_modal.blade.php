...
<div class="modal-body">
    <form action="{{ url('setting/user-group/save', [$userGroup?->id ?? 0]) }}" method="POST"
        enctype="multipart/form-data" id="form_validation">

        <div class="row mb-3">
            <label for="code" class="col-sm-12 col-md-12 col-form-label">Kode</label>
            <div class="col-sm-12 col-md-12">
                <input type="text" name="code" class="form-control" id="code" value="{{ $userGroup?->code }}"
                    required>
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-12 col-md-12">
                <input type="text" name="name" class="form-control" id="name" value="{{ $userGroup?->name }}"
                    required>
            </div>
        </div>
                        ...
                        @foreach ($statuses as $key => $value)
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status"
                                    ...
                                    {{ ($userGroup?->status ?? 'active') == $key ? 'checked' : '' }}>
        ...
        $('#form_validation').on('submit', function(e) {
            ...
            $.ajax({
                ...
                let errors = xhr.responseJSON?.errors ?? xhr.responseJSON?.message ??
                    "Terjadi masalah, coba beberapa saat lagi";
