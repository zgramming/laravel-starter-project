<div class="modal-header-custom p-4" style="border-bottom: 1px solid #dee2e6;">
    <div class="d-flex flex-row justify-content-between align-items-end">
        <h4 class="modal-title" id="modal-default-label">{{ empty($parameter) ? "Form Tambah" : "Form Update" }}</h4>
        <button type="button" class="btn btn-light-secondary" data-bs-dismiss="modal" aria-label="Close"><i class="fa fa-times"></i></button>
    </div>
</div>

<div class="modal-body">
    <form action="{{ url("setting/parameter/save",[$parameter?->id ?? 0]) }}" method="POST" enctype="multipart/form-data" id="form_validation">

        <div class="row mb-3">
            <label for="code" class="col-sm-12 col-md-12 col-form-label">Kode</label>
            <div class="col-sm-12 col-md-12">
                <input type="text" name="code" class="form-control" id="code" value="{{$parameter?->code}}" required >
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-12 col-md-12">
                <input type="text" name="name" class="form-control" id="name" value="{{$parameter?->name}}" required >
            </div>
        </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-sm-12 col-md-12">
                <input type="text" name="value" class="form-control" id="value" value="{{$parameter?->value}}" required >
            </div>
        </div>
                    ...
                    <div class="radio-container">
                        @foreach($statuses as $key => $value)
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="status" id="status_{{$key}}" value="{{$key}}" {{ ($parameter?->status ?? 'active') == $key ? "checked" : ""}}>
...
<script type="text/javascript">
    $(document).ready(function(e){
        ...
        $('#form_validation').on('submit',function(e){
            ...
            let url = `{{ url("setting/parameter/save",[$parameter?->id ?? 0]) }}`;
            $.ajax({
                url : url,
                method : 'POST',
                data : data,
